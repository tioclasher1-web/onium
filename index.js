...
// Coba konek ke Supabase hanya jika credentials ada
...
// ========== API ENDPOINTS ==========
// Health check endpoint (penting untuk debug)
app.get('/api/health', (req, res) => {
    res.json({
        status: 'ok',
        supabase: useSupabase ? 'connected' : 'disabled',
...
<head>
    ...
    <style>
        *{margin:0;padding:0;box-sizing:border-box}
        ...
        .sidebar{width:280px;background:linear-gradient(180deg,#2c3e50 0%,#1a1a2e 100%);color:#fff;padding:30px 20px}
        .sidebar h2{text-align:center;margin-bottom:30px}
        .nav-link{display:flex;align-items:center;padding:12px 20px;color:#ecf0f1;border-radius:10px;cursor:pointer;margin-bottom:10px}
        ...
        .card{background:#fff;border-radius:15px;padding:25px;margin-bottom:25px;box-shadow:0 10px 30px rgba(0,0,0,0.1)}
        ...
        button{background:linear-gradient(135deg,#667eea 0%,#764ba2 100%);color:#fff;padding:12px 30px;border:none;border-radius:8px;cursor:pointer}
        .key-result{margin-top:20px;padding:20px;background:linear-gradient(135deg,#f5f7fa 0%,#c3cfe2 100%);border-radius:10px;display:none}
        .key-result.show{display:block}
        .key-display{font-family:monospace;font-size:20px;font-weight:bold;color:#667eea;margin:10px 0}
        ...
        .delete-btn{background:#dc3545;color:#fff;border:none;border-radius:6px;padding:6px 12px;cursor:pointer}
        ...
        @media (max-width:768px){.sidebar{width:80px;padding:20px 10px}.sidebar h2,.nav-link span:not(.emoji){display:none}.nav-link .emoji{margin-right:0;font-size:24px}}
    </style>
</head>
<body>
    <div class="container">
        ...
        <div class="main-content">
            <div id="dashboard" class="page active">
                ...
                <div class="card">
                    ...
                    <div class="table-container">
                        <table id="keysTable"><thead><tr><th>Key</th><th>Device Used</th><th>Sisa Hari</th><th>Status</th><th>Aksi</th></tr></thead>
                        <tbody id="keysTableBody"><tr><td colspan="5">Loading...</td></tr></tbody></table>
                    </div>
                </div>
            </div>
            <div id="getkey" class="page">
                <div class="card">
                    ...
                    <div id="keyResult" class="key-result"><h4>✅ Key Berhasil Dibuat!</h4><div class="key-display" id="generatedKey"></div><p><strong>Masa Aktif:</strong> <span id="resultDays"></span> hari</p><p><strong>Max Device:</strong> <span id="resultDevices"></span> device</p><p>🎉 Terima kasih sudah membeli VIP kami!</p></div>
            ...
            <div id="reset" class="page">
                <div class="card">
                    ...
                    <div style="display:flex;gap:10px"><input type="text" id="resetKeyInput" placeholder="Masukkan Key" style="flex:1;padding:12px;border:2px solid #e0e0e0;border-radius:8px"><button id="resetBtn">Reset Device</button></div>
    ...
    <script>
            ...
            try{
                ...
                if(Object.keys(keys).length===0){
                    ...
                    for(const[key,data]of Object.entries(keys)){
                        ...
                        tbody.innerHTML+='<tr><td><strong>'+key+'</strong></td><td>'+data.usedDevices+'/'+data.maxDevices+'</td><td>'+data.remainingDays+'</td><td><span class="status-badge '+statusClass+'">'+data.status+'</span></td><td><button class="delete-btn" onclick="deleteKey(\''+key+'\')">Hapus</button></td></tr>';
                    }
                }
        ...
        document.getElementById('generateKeyForm').addEventListener('submit',async(e)=>{
            ...
            const res=await fetch('/api/generate-key',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({activeDays,maxDevices})});
        ...
        document.getElementById('resetBtn').addEventListener('click',async()=>{
            ...
            if(confirm('Reset device untuk key '+key+'?')){
                const res=await fetch('/api/reset-key',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({key})});
                const result=await res.json();
                if(result.success){
        ...
        loadKeys();
        setInterval(()=>{if(document.getElementById('dashboard').classList.contains('active'))loadKeys();},10000);
    </script>
</body>
</html>`);
});
